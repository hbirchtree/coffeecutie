if(APPLE)
  set(OSX_DEFAULT_ICON
      "${COFFEE_DESKTOP_DIRECTORY}/osx/Coffee.icns"
      CACHE FILEPATH ""
  )
  set(IOS_LOADING_ICON
      "${COFFEE_DESKTOP_DIRECTORY}/common/icon_large.png"
      CACHE FILEPATH ""
  )
  set(IOS_DEFAULT_STORYBOARD
      "${COFFEE_DESKTOP_DIRECTORY}/osx/Coffee.storyboard"
      CACHE FILEPATH ""
  )
  set(IOS_ICON_ASSETS
      "${COFFEE_DESKTOP_DIRECTORY}/osx/Icons.xcassets"
      CACHE PATH ""
  )
...
macro(
  ...
  # TODO: Process resource files TODO: Process header files

  # install( TARGETS "${TARGET}"

  # EXPORT ${PROJECT_NAME}

  # LIBRARY DESTINATION lib ARCHIVE DESTINATION lib FRAMEWORK DESTINATION
  # frameworks PUBLIC_HEADER DESTINATION include
...
macro(
  ...
  set_source_files_properties(
    ${OSX_ICON} PROPERTIES MACOSX_PACKAGE_LOCATION "${RESOURCE_DIR}"
  )
  ...
  if(IOS)
    ...
    configure_file(
      "${COFFEE_DESKTOP_DIRECTORY}/osx/ios/Info.plist.xml" "${IOS_PLIST_FILE}"
      @ONLY
    )
    ...
    set_target_properties(
      ${TARGET}
      PROPERTIES MACOSX_BUNDLE_BUNDLE_NAME "${TITLE}"
    ...
    set(RESOURCE_DIR ${CMAKE_BINARY_DIR}/bin/${TARGET}.app/Contents/Resources)

    get_filename_component(ICON_BASENAME "${OSX_ICON}" NAME)
    add_custom_target(
      ${TARGET}.resources COMMAND ${CMAKE_COMMAND} -E make_directory
                                  ${RESOURCE_DIR}
    )
    add_custom_target(
        ${TARGET}.iconset
  ...
  if(IOS)
    install(TARGETS ${TARGET}
            DESTINATION ${CMAKE_PACKAGED_OUTPUT_PREFIX}/apple-ios
    )
  else()
    install(TARGETS ${TARGET}
            DESTINATION ${CMAKE_PACKAGED_OUTPUT_PREFIX}/apple-osx
    )
  endif()
endmacro()
