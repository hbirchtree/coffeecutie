if("${CMAKE_SYSTEM_NAME}" STREQUAL "Linux")
  ...
  set(SNAPPY_CONFIG_DIR
      "${COFFEE_DESKTOP_DIRECTORY}/linux/snappy"
      CACHE PATH ""
  )
...
macro(SNAPPY_TRANSLATE_ARCH_NAME OUTPUT_VAR)
  ...
  elseif("${CMAKE_LIBRARY_ARCHITECTURE}" STREQUAL "i386-linux-gnu"
...
macro(SNAPPY_TRANSLATE_PERMISSIONS PERMISSIONS_LIST PERMISSION_OUTPUT)
  ...
  foreach(PARAM ${PERMISSIONS_LIST})
    ...
    if("${PARAM_ENABLED}" STREQUAL "1")
      set(PARAM_TRANS)
      # Snapcraft does not like it if we have non-unique elements in the list,
      # we therefore verify this
      ...
      elseif("${PARAM}" MATCHES "NETWORK_CONNECT" OR "${PARAM}" MATCHES
                                                     "NETWORK_SERVE"
      )
...
macro(
  ...
  set(SNAPPY_FINAL_SNAP
      "${SNAPPY_OUTPUT_DIRECTORY}/${SNAPPY_PACKAGE_NAME}_${VERSION}_all.snap"
  )
  ...
  configure_file("${SNAPPY_CONFIG_DIR}/snap.yaml.in" "${SNAPCRAFT_FILE}" @ONLY)

  configure_file(
    "${SNAPPY_CONFIG_DIR}/snap-select.sh" "${SNAPPY_PKG_DIR}/snap-select.sh"
    @ONLY
  )

  file(APPEND "${SNAPCRAFT_FILE}" "apps:\n" "  game:\n"
                                  "    command: bin/${TARGET}\n" "    plugs:\n"
  )
  foreach(PERM ${SNAPPY_PERMISSIONS})
    file(APPEND "${SNAPCRAFT_FILE}" "      - ${PERM}\n")
  endforeach()
  file(
    APPEND "${SNAPCRAFT_FILE}"
    "parts:\n"
    "  binary-import:\n"
    "    plugin: copy\n"
    "    stage-packages:\n"
  ...
  target_sources(
    ${TARGET}.snap PUBLIC "${SNAPCRAFT_FILE}" "${ICON_TARGET}"
  ...
  add_custom_command(
    TARGET ${TARGET}.snap
    POST_BUILD
    COMMAND ${PYTHON_ENCODING_STUFF} ${SNAPPY_PROGRAM} snap -o
            "${SNAPPY_FINAL_SNAP}"
    WORKING_DIRECTORY ${SNAPPY_PKG_DIR}
    USES_TERMINAL
  )

  install(FILES "${SNAPPY_FINAL_SNAP}"
          DESTINATION "${CMAKE_PACKAGED_OUTPUT_PREFIX}/${SNAPPY_PACKAGING_CAT}"
  )
