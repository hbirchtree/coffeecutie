if("${CMAKE_SYSTEM_NAME}" STREQUAL "Linux")
  ...
  set(APPIMAGE_RUNTIME_BINARY
      CACHE FILEPATH "AppImage runtime to be embedded into the AppImage"
  )

  set(APPIMAGE_EXTRA_LIBRARIES CACHE STRING "")

  set(APPIMAGE_WORKING_DIRECTORY
      "${COFFEE_DEPLOY_DIRECTORY}/linux-appimage"
      CACHE PATH "Where to put the AppDir items"
  )
  set(APPIMAGE_OUTPUT_DIRECTORY
      "${COFFEE_PACKAGE_DIRECTORY}/linux-appimage"
      CACHE PATH "AppImage output directory"
  )
  set(APPIMAGE_FOLLOW_STANDARD
      OFF
...
function(
  ...
  configure_file(
    ...
    "${APPIMAGE_INTERMEDIATE_DIR}/set_icon.py" COPYONLY
  )
  configure_file(
    "${APPIMAGE_CONFIG_DIR}/application.desktop.in"
    "${APPIMAGE_INTERMEDIATE_DIR}/${APPIMAGE_INTERNALNAME}.desktop" @ONLY
  )
  if(NOT APPIMAGE_FOLLOW_STANDARD)
    file(
    ...
    configure_file(
      ...
      "${APPIMAGE_INTERMEDIATE_DIR}/AppRun" @ONLY
    )
  else()
    configure_file(
      "${APPIMAGE_APPRUN_PROGRAM}" "${APPIMAGE_INTERMEDIATE_DIR}/AppRun"
      COPYONLY
    )
  endif()
