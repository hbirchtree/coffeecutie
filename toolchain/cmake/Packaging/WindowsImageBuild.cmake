macro(
  ...
  if(WIN_UWP)
    target_include_directories(
      ${TARGET} PRIVATE $<BUILD_INTERFACE:${SDL2_INCLUDE_DIR}>
    )
    target_include_directories(
      ${TARGET} PRIVATE $<BUILD_INTERFACE:${ANGLE_INCLUDE_DIR}>
    )

    # I need these seeds for mye research, Morty, gotta stuff it waaay up there,
    # Morty.
    target_link_libraries(${TARGET} PUBLIC ${SDL2_LIBRARY})
    set_source_files_properties(
      ${SDL2_MAIN_C_FILE} PROPERTIES COMPILE_FLAGS /ZW
    )
    # set_target_properties ( ${TARGET} PROPERTIES RESOURCE
    # "${SDL2_LIBRARY_BIN};${ANGLE_LIBRARIES_BIN}" )

    execute_process(
      COMMAND cmake -E make_directory ${CMAKE_CURRENT_BINARY_DIR}/${TARGET}.dir
    )
    ...
    foreach(durr ${RESOURCES})
      ...
      foreach(file_full ${TMP})
        ...
        if(NOT ("${file_name_lower}" STREQUAL "thumbs.db"))
          ...
          set_source_files_properties(
            "${file_full}" PROPERTIES VS_DEPLOYMENT_CONTENT 1
    ...
    foreach(var ${INCLUDED_LIBS})
      set_source_files_properties(
        "${var}" PROPERTIES VS_DEPLOYMENT_CONTENT 1 VS_DEPLOYMENT_LOCATION "."
      )
    endforeach()
  endif()
  ...
  if(NOT WIN_UWP)
    ...
    install(DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/AppPackages/${TARGET}
