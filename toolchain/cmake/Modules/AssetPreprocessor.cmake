...
function(COFFEE_PROCESS_ASSETS)
  cmake_parse_arguments(
    ASS "" "TARGET;OUTPUT_DIRECTORY;INPUT_DIRECTORY" "SHADER_VERSIONS" ${ARGN}
  )
    ...
    if(${REL_FILE} MATCHES "\.(vert|frag|comp|tess)$")
      foreach(VER ${ASS_SHADER_VERSIONS})
        ...
        if(${VER} STREQUAL "spv")
          add_custom_command(
            ...
            PRE_BUILD
            COMMAND
              ${CMAKE_BINARY_DIR}/bin/ShaderCooker ${FILE} -f -B -o
              ${ASS_OUTPUT_DIRECTORY}/${DIR}/${BASENAME}${EXTENSION}.spv
              DEPENDS ${FILE}
          )
          continue()
        endif()
        add_custom_command(
          ...
          PRE_BUILD
          COMMAND
            ${CMAKE_BINARY_DIR}/bin/ShaderCooker ${FILE} -f -p ${PROFILE} -V
            ${VERSION} -o
            ${ASS_OUTPUT_DIRECTORY}/${DIR}/${BASENAME}.${PROFILE}${VERSION}${EXTENSION}
            DEPENDS ${FILE}
        )
      endforeach()
    else()
      add_custom_command(
        ...
        COMMAND ${CMAKE_COMMAND} -E copy ${FILE}
                ${ASS_OUTPUT_DIRECTORY}/${REL_FILE} DEPENDS ${FILE}
      )
    endif()
  endforeach()
endfunction()
