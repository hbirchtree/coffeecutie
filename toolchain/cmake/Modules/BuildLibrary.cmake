...
macro(EXTRACT_HEADER_DIR INPUT_DIR OUTPUT_VAR)
  string(REGEX REPLACE "^.*(INSTALL|BUILD)_INTERFACE:(.*)\>+$" "\\2"
                       ${OUTPUT_VAR} "${INPUT_DIR}"
  )
endmacro()
  ...
  if(NOT "${INC_DIR}" STREQUAL "")
    foreach(INC ${INC_DIR})
      if("${INC}" MATCHES ".*BUILD_INTERFACE:.*")
        ...
        string(REGEX REPLACE ".*\/([^\/]*include)$" "\\1" BASE_SUFFIX "${INC}")
        if(IS_ABSOLUTE "${BASE_SUFFIX}")
          set(BASE_SUFFIX include)
        endif()
        target_include_directories(
          ${LIBNAME}
          INTERFACE $<INSTALL_INTERFACE:targets/${LIBNAME}/${BASE_SUFFIX}>
        )
      endif()
    endforeach()
...
function(ADD_EXPORT LIB_TARGET)
  if(ANDROID)
    ...
    install(
      ...
      PUBLIC_HEADER DESTINATION include/${LIB_TARGET} COMPONENT bin
    )
  endif()
endfunction()
  ...
  foreach(HEADER_DIR ${LIB_HEADER_DIRS})
    if("${HEADER_DIR}" MATCHES ".*BUILD_INTERFACE:.*")
      ...
      if(IS_DIRECTORY "${HEAD_DIR}")
        file(GLOB_RECURSE ALL_HEADERS ${HEAD_DIR}/*.h ${HEAD_DIR}/*.hpp)
      else()
...
macro(COFFEE_FRAMEWORK)
  cmake_parse_arguments(
    LIB "" "TARGET;LINKAGE;VERSION_CODE;COPYRIGHT;COMPANY"
