...
function(DEPENDENCY_GET)
  ...
  cmake_parse_arguments(
    DEP "REQUIRED" "EXTENSION;SOURCE;TAG;CACHE_LOCATION;LIB_LOCATION;SIDELOAD"
    "NAMES" ${ARGN}
  )
  ...
  execute_process(
    COMMAND ${CMAKE_COMMAND} -E make_directory "${DEP_CACHE_LOCATION}"
  )

  set(BASE_URL "https://github.com/${DEP_SOURCE}/releases/download/${DEP_TAG}")
  ...
  foreach(NAME_COMBO ${DEP_NAMES})
    ...
    set("GIT_DEPENDENCIES_${NAME_COMBO}_VERSION"
        ${DEP_TAG}
        PARENT_SCOPE
    )
    set("GIT_DEPENDENCIES_${NAME_COMBO}_SOURCE"
        ${DEP_SOURCE}
        PARENT_SCOPE
    )
    set("GIT_DEPENDENCIES_${NAME_COMBO}_EXTENSION"
        ${DEP_EXTENSION}
        PARENT_SCOPE
    )
    ...
    #
    # In order to avoid globbing up the CMAKE_PREFIX_PATH, we only append it
    # when it's not there. We also have to use FORCE in order to make this work
    # properly. This is probably very bad, but lets FIND_PACKAGE() and
    # pkg-config work
    #
    if(NOT "${CMAKE_PREFIX_PATH}" MATCHES "${LOCAL_DIR}")
      set(CMAKE_PREFIX_PATH "${CMAKE_PREFIX_PATH};${LOCAL_DIR}")

      if(DEFINED CMAKE_FIND_ROOT_PATH)
        set(CMAKE_FIND_ROOT_PATH "${CMAKE_FIND_ROOT_PATH};${LOCAL_DIR}")
      endif()
    endif()
    ...
    file(DOWNLOAD "${FILE_URL}" "${LOCAL_FILE}" STATUS DL_STAT)
    ...
    #
    execute_process(COMMAND ${CMAKE_COMMAND} -E make_directory "${LOCAL_DIR}")
...
function(DEPENDENCY_LINK)
  ...
  cmake_parse_arguments(LINK "" "TARGET" "DEPENDENCIES" ${ARGN})
  ...
  set_property(TARGET "${LINK_TARGET}" PROPERTY EXPORT_PROPERTIES git_deps)
endfunction()
...
macro(DEPENDENCY_RESOLVE_INTERNAL)
  ...
  cmake_parse_arguments(RES "" "TARGET" "" ${ARGN})
