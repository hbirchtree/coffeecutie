...
function(COFFEE_APPLICATION)
  ...
  # TARGET TITLE APP_COMPANY_NAME APP_VERSION_CODE INFO_STRING COPYRIGHT SOURCES
  # LIBRARIES BUNDLE_LIBS BUNDLE_RSRCS BUNDLE_LICENSES PERMISSIONS
  # BUNDLE_BINARIES
  ...
  set(oneOpts
      ...
      INFO_STRING
      COPYRIGHT
      # Package name generation PACKAGE_PREFIX is prepended to target name
      PACKAGE_PREFIX
  ...
  set(multiOpts
      ...
      BUNDLE_LIBRARIES
      # Just resources used by the program Specified as the root of a resource
      # directory
      RESOURCES
  ...
  set(switches # Indicates building a console application. Mostly used on OSX to
      # disable building as .app. Will output plain binary on OSX.
      USE_CMD
  )

  cmake_parse_arguments(APP "${switches}" "${oneOpts}" "${multiOpts}" ${ARGN})
  ...
  coffee_gen_applicationinfo(
    "${APP_TARGET}" "${APP_TITLE}" "${APP_COMPANY}" "${APP_VERSION_CODE}"
  )
  ...
  elseif(${CMAKE_SYSTEM_NAME} STREQUAL "Linux")
    ...
    # add_custom_command ( OUTPUT ${EMBED_SRC} COMMAND ${PYTHON_COMMAND}
    # ${CMAKE_SOURCE_DIR}/toolchain/resources-to-source.py --output=${EMBED_SRC}
    # ${EMBEDDED_RESOURCES} DEPENDS ${EMBEDDED_RESOURCES} ) target_sources (
    # ${APP_TARGET} PUBLIC ${EMBED_SRC} ) add_custom_command ( TARGET
    # ${APP_TARGET} PRE_BUILD COMMAND ${CMAKE_COMMAND} -E remove ${EMBED_SRC} )

    # We still install a basic binary executable
    install(FILES "$<TARGET_FILE:${APP_TARGET}>"
    ...
    file(
      WRITE "${CMAKE_CURRENT_BINARY_DIR}/${APP_TARGET}.desktop"
      "[Desktop Entry]
Version=1.0
Type=Application
Name=${APP_TITLE}
...
"
  ...
  set_target_properties(
    ${APP_TARGET}
    PROPERTIES APP_TITLE "${APP_TITLE}"
               APP_COLOR "#E91E63"
  ...
  target_compile_definitions(
    ${APP_TARGET} PRIVATE -DCOFFEE_COMPONENT_NAME="${APP_TARGET}"
