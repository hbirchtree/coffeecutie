#pragma once

#include <coffee/core/CTypes>

namespace Coffee{
namespace NoiseGen{

/*!
 * \brief Returns perlin noise values. Should operate in a thread-safe manner.
 * \param pos Position of sample
 * \param wrap Wrapping size of perlin noise, 256 is the maximum effective value
 * \return The noise value
 */
//extern scalar Perlin(
//        const CVec3& pos,
//        const _cbasic_vec3<int32>& wrap = _cbasic_vec3<int32>());

namespace PlFun{

static uint8 plnoise[] = {
    151,160,137,91,90,15,
    131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
    190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,
    88,237,149,56,87,174,20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,
    77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,
    102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,18,169,200,196,
    135,130,116,188,159,86,164,100,109,198,173,186, 3,64,52,217,226,250,124,123,
    5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,
    223,183,170,213,119,248,152, 2,44,154,163, 70,221,153,101,155,167, 43,172,9,
    129,22,39,253, 19,98,108,110,79,113,224,232,178,185, 112,104,218,246,97,228,
    251,34,242,193,238,210,144,12,191,179,162,241, 81,51,145,235,249,14,239,107,
    49,192,214, 31,181,199,106,157,184, 84,204,176,115,121,50,45,127, 4,150,254,
    138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
};

template<typename T>
inline C_FORCE_INLINE T fade(T v)
{
    return CMath::pow<T>(v,3)*(v*(v*6-15)+10);
}

template<typename T,typename W>
inline C_FORCE_INLINE T grad(W hash, T x, T y, T z)
{
    W h = hash&15;
    T u = h<8 ? x : y;
    T v = h<4 ? y : (h==12||h==14) ? x : z;
    return ((h&1)==0 ? u : v) + ((h&2)==0 ? v : -v);
}

}

template<typename T,typename W,size_t Size>
inline C_FORCE_INLINE scalar Perlin(
        const _cbasic_tvector<T,Size>& pos,
        const _cbasic_tvector<W,Size>& wrap = _cbasic_tvector<W,Size>())
{
    using namespace CMath;
    using namespace PlFun;
    uint8* p = plnoise;
    T x = pos.x(),y=pos.y(),z=pos.z();

    W mx = (W)ceil(pos.x()) & wrap.x();
    W my = (W)ceil(pos.y()) & wrap.y();
    W mz = (W)ceil(pos.z()) & wrap.z();

    T u = fade(pos.x()),v = fade(pos.y()),w = fade(pos.z());

    W a = plnoise[mx]+my;
    W aa = plnoise[a]+mz;
    W ab = plnoise[a+1]+mz;
    W b = plnoise[mx+1]+my;
    W ba = plnoise[b]+mz;
    W bb = plnoise[b+1]+mz;

    return mix(w, mix(v,
                      mix(u, grad(p[aa],x,y,z), grad(p[ba],x-1,y,z)),
                      mix(u, grad(p[ab],x,y-1,z), grad(p[bb],x-1,y-1,z))),
                  mix(v,
                      mix(u, grad(p[aa+1],x,y,z-1), grad(p[ba+1],x-1,y,z-1)),
                      mix(u, grad(p[ab+1],x,y-1,z-1), grad(p[bb+1],x-1,y-1,z-1)))
            );
}

/*!
 * \brief Makes transfers the floating-point range [0.0,1.0] to integer [0,255]
 * \param v Value to transform, scaled to [0,1] range
 * \return A linear value in uint8 format
 */
inline C_FORCE_INLINE uint8 Linearize(scalar v)
{
    return (uint8)(v*255);
}

}
}