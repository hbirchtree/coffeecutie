...
std::optional<std::pair<std::string, std::string>> wine_host_system()
{
    ...
    /* Source:
     * https://www.winehq.org/pipermail/wine-devel/2008-September/069387.html */
    typedef void(CDECL * WINE_GET_HOST_VERSION_FPTR)(
        const char**, const char**);

    static WINE_GET_HOST_VERSION_FPTR pwine_get_host_version = nullptr;
    ...
    if(!pwine_get_host_version)
    {
        ...
        if(!hntdll)
            return std::nullopt;
        pwine_get_host_version = (WINE_GET_HOST_VERSION_FPTR)GetProcAddress(
            hntdll, "wine_get_host_version");
        if(!pwine_get_host_version)
            return std::nullopt;
    }

    const char* system_name{nullptr};
    const char* system_version{nullptr};
    pwine_get_host_version(&system_name, &system_version);
    if(system_name && system_version)
        return std::make_pair(
            std::string(system_name), std::string(system_version));
    else
        return std::nullopt;
}
