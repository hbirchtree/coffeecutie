...
struct tquaternion : tvector<T, 4>
{
    ...
    tquaternion<T> operator*(const tquaternion<T>& v) const
    {
        ...
        vnew.w() = this->w() * v.w() - this->x() * v.x() - this->y() * v.y() -
                   this->z() * v.z();
        vnew.x() = this->w() * v.x() + this->x() * v.w() + this->y() * v.z() -
                   this->z() * v.y();
        vnew.y() = this->w() * v.y() - this->x() * v.z() + this->y() * v.w() +
                   this->z() * v.w();
        vnew.z() = this->w() * v.z() + this->x() * v.y() - this->y() * v.x() +
                   this->z() * v.x();
    ...
    STATICINLINE
    tquaternion<T> from_euler(const tvector<T, 3>& euler)
    {
...
template<typename T>
FORCEDINLINE T dot(const tquaternion<T>& v1, const tquaternion<T>& v2)
{
    tvector<T, 4> tmp;
    tmp[0] = v1.x() * v2.x();
    tmp[1] = v1.y() * v2.y();
...
template<typename T>
FORCEDINLINE T length(const tquaternion<T>& v)
{
    return sqrt(dot(v, v));
}

template<typename T>
FORCEDINLINE tquaternion<T> normalize_quat(const tquaternion<T>& v)
{
    T len = length(v);
    if(len <= T(0))
...
FORCEDINLINE tvector<T, 3> quaternion_to_direction(tquaternion<T> const& q)
{
