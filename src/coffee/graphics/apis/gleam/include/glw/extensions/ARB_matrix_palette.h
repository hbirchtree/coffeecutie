...
requires(semantic::concepts::Span<span_const_u8>&& std::is_same_v<
    ...
    void matrix_indexubv(span_const_u8 const& indices)
{
    using namespace std::string_view_literals;
    if constexpr(compile_info::debug_mode)
    {
...
requires(semantic::concepts::Span<span_const_u32>&& std::is_same_v<
    ...
    void matrix_indexuiv(span_const_u32 const& indices)
{
    using namespace std::string_view_literals;
    if constexpr(compile_info::debug_mode)
    {
...
requires(semantic::concepts::Span<span_const_u16>&& std::is_same_v<
    ...
    void matrix_indexusv(span_const_u16 const& indices)
{
    using namespace std::string_view_literals;
    if constexpr(compile_info::debug_mode)
    {
