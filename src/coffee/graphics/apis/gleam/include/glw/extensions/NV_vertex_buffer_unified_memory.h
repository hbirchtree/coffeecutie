 ...
 * \return void
 */
STATICINLINE void edge_flag_format(
 ...
 * \return void
 */
STATICINLINE void fog_coord_format(
    ...
    error_check      check_errors = error_check::on)
{
    ...
    glGetIntegerui64i_vNV(
        ...
        result.size() ? reinterpret_cast<GLuint64EXT*>(result.data())
                      : nullptr);
    detail::error_check("GetIntegerui64i_vNV"sv, check_errors);
}
 ...
 * \return void
 */
STATICINLINE void index_format(
    GLenum type, i32 stride, error_check check_errors = error_check::on)
{
    using namespace std::string_view_literals;
    if constexpr(compile_info::debug_mode)
 ...
 * \return void
 */
STATICINLINE void normal_format(
    GLenum type, i32 stride, error_check check_errors = error_check::on)
{
    using namespace std::string_view_literals;
    if constexpr(compile_info::debug_mode)
 ...
 * \return void
 */
STATICINLINE void tex_coord_format(
    i32         size,
    GLenum      type,
    i32         stride,
    error_check check_errors = error_check::on)
