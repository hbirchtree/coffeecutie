...
HWDeviceInfo RaspberrySysInfo::DeviceName()
{
    ...
    do
    {
        ...
        cstring match = StrFind(cmdline_s,boardrev_string);
        if(!match)
            break;
        cstring val = Search::ChrFind(match,'=');
        if(!val)
            break;
        val += 1;
        cstring end = Search::ChrFind(val,' ');
        if(!end || (end-val) <= 0)
            break;
        ...
        /* Pi A */
        else if(firmware_string == "0x7" || firmware_string == "0x8" || firmware_string == "0x9")
            model_string = model_names[0];
        /* Pi B */
        else if(firmware_string == "0x2" || firmware_string == "0x3" || firmware_string == "0x4"
                || firmware_string == "0x5" || firmware_string == "0x6")
        ...
        /* Pi 2 B */
        else if(firmware_string == "0xa01040" || firmware_string == "0xa01041"
...
{
    bigscalar freq = 0.0;
    CString freq_s = FileFun::sys_read("/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq");

    if(freq_s.size() > 0)
        freq = Convert::strtoscalar(freq_s.c_str()) / 1000000;
