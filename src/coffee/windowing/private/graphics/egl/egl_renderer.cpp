...
#define EGL_NULLCHECK(result, error_text)                    \
    if(!result)                                              \
    {                                                        \
        cVerbose(EGL_MIN_VERB, "@" #result ": " error_text); \
        if(err)                                              \
            *err = error_text;                               \
        return false;                                        \
    }
...
#define EGL_VERIFYCONTEXT()                                  \
    if(!m_eglData || m_eglData->display == EGL_NO_DISPLAY || \
...
#define EGL_PRINT_VALUE(property)                                     \
    ...
    cVerbose(                                                         \
        EGL_MIN_VERB,                                                 \
        "Config {0}: EGL property" #property ": {1}",                 \
        EGL_PRINT(cfg),                                               \
        prop);

bool egl_config_to_bits(EGLDisplay disp, EGLConfig cfg, ContextBits& bits)
{
    EGLint prop = 0;
...
static bool egl_create_context(
    EGLRenderer* renderer, EGL_Data* m_eglData, CString* err)
{
    {
        ...
        m_eglData->context = eglCreateContext(
            m_eglData->display,
            m_eglData->config,
            EGL_NO_CONTEXT,
            &config_preferred[0]);
    ...
    cVerbose(
        8,
        "X11 display connection from EGL: {0}",
    ...
    m_eglData->surface = eglCreateWindowSurface(
        m_eglData->display,
        m_eglData->config,
        (EGLNativeWindowType)x_win,
        nullptr);
#elif defined(COFFEE_RASPBERRY_DMX)
    m_eglData->surface = eglCreateWindowSurface(
        m_eglData->display, m_eglData->config, &dmx_win, nullptr);
#elif defined(COFFEE_USE_WINDOWS_ANGLE)
    m_eglData->surface = eglCreateWindowSurface(
        m_eglData->display, m_eglData->config, egl_win, nullptr);
#elif defined(COFFEE_USE_APPLE_GLKIT)
    // The display is holding information on context and views in the
    // Objective-C code
    m_eglData->surface = eglCreateWindowSurface(
        m_eglData->display, m_eglData->config, nullptr, nullptr);
    ...
    cVerbose(
        8, "EGL vendor: {0}", eglQueryString(m_eglData->display, EGL_VENDOR));
    cVerbose(
        8,
        "EGL extensions: {0}",
        eglQueryString(m_eglData->display, EGL_EXTENSIONS));
    cVerbose(
        8, "EGL version: {0}", eglQueryString(m_eglData->display, EGL_VERSION));
    cVerbose(
        8,
        "EGL client APIs: {0}",
...
struct EGL_GL_Context : public CGL::CGL_Context, public CObject
{
  ...
  public:
    EGL_GL_Context(EGLRenderer& renderer) :
        CObject(nullptr), data(*renderer.m_eglData)
    {
    }

    virtual bool acquireContext()
    {
        EGL_ERRORCHECK_N(eglMakeCurrent(
    ...
    {
        EGL_ERRORCHECK_N(
            eglMakeCurrent(data.display, nullptr, nullptr, nullptr));
        return true;
    }
...
struct EGL_GL_WorkerContext : public CGL::CGL_WorkerContext
{
  ...
  public:
    EGL_GL_WorkerContext(EGLRenderer& renderer) : data()
    {
...
bool EGLRenderer::contextPreInit(const GL::Properties&, CString*)
{
    return true;
}

bool EGLRenderer::contextInit(const GL::Properties&, CString*)
{
    return true;
}

bool EGLRenderer::contextPostInit(const GL::Properties& props, CString* err)
{
...

    {
        ...
        eglChooseConfig(
            m_eglData->display, &config_preferred[0], nullptr, 0, &config_num);
        ...
        EGL_ERRORCHECK(eglChooseConfig(
            m_eglData->display,
            &config_preferred[0],
            &configurations[0],
            config_num,
        ...
        for(EGLConfig const& cfg : configurations)
        {
            ...
            if(bits.samples == props.bits.samples &&
               bits.red == props.bits.red && bits.green == props.bits.green &&
...
void EGLRenderer::swapBuffers()
{
    ...
    EGL_ERRORCHECK_NOBOOL(
        eglSwapBuffers(m_eglData->display, m_eglData->surface));
}

Size EGLRenderer::framebufferSize() const
{
    Size size = {};
    ...
    EGL_ERRORCHECK_NOBOOL(eglQuerySurface(
        m_eglData->display,
        m_eglData->surface,
        EGL_WIDTH,
        C_RCAST<EGLint*>(&size.w)));
    EGL_ERRORCHECK_NOBOOL(eglQuerySurface(
        m_eglData->display,
        m_eglData->surface,
        EGL_HEIGHT,
        C_RCAST<EGLint*>(&size.h)));
...
void EGLRenderer::setSwapInterval(const int& i)
{
    ...
    {
        cWarning(
            "Invalid swap interval, limits: [{0}, {1}], got {2}",
            min_swap,
            max_swap,
            i);
        return;
...
ContextBits EGLRenderer::context()
{
    ContextBits bits;
