...
#define LOGI(...)                                           \
    ((void)__android_log_print(                             \
        ANDROID_LOG_INFO,                                   \
        ndk_helper::JNIHelper::GetInstance()->GetAppName(), \
        __VA_ARGS__))
#define LOGW(...)                                           \
    ((void)__android_log_print(                             \
        ANDROID_LOG_WARN,                                   \
...
class JNIHelper
{
  private:
    ...
    // This class uses singleton pattern and can be invoked from multiple
    // threads, each methods locks the mutex for a thread safety
  ...
  public:
    /*
     * To load your own Java classes, JNIHelper requires to be initialized with
     * a ANativeActivity handle. This methods need to be called before any call
     * to the helper class. Static member of the class
     *
     * arguments:
     * in: activity, pointer to ANativeActivity. Used internally to set up JNI
     * environment in: helper_class_name, pointer to Java side helper class
     * name. (e.g. "com/sample/helper/NDKHelper" in samples )
     */
     ...
     * file when the call failed, contents of the buffer remains same return:
     ...
     * The method invokes BitmapFactory in Java so it can read jpeg/png
     * formatted files
     *
     ...
     * GL_TEXTURE_MAG_FILTER, GL_LINEAR ); glGenerateMipmap( GL_TEXTURE_2D );
     *
     * arguments:
    ...
    /*
     * Audio helper
     * Retrieves native audio buffer size which is required to achieve low
     * latency audio
     *
     * return: Native audio buffer size which is a hint to achieve low latency
     * audio If the API is not supported (API level < 17), it returns 0
     */
    int32_t GetNativeAudioBufferSize();

    /*
     * Audio helper
     * Retrieves native audio sample rate which is required to achieve low
     * latency audio
     *
