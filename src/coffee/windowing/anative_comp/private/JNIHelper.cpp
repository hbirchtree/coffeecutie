...
#include <coffee/anative/JNIHelper.h>
...
void JNIHelper::Init(ANativeActivity* activity, const char* helper_class_name)
{
    ...
    jclass android_content_Context =
        env->GetObjectClass(helper.activity_->clazz);
    ...
    jstring packageName = (jstring)env->CallObjectMethod(
        helper.activity_->clazz, midGetPackageName);
...
// readFile
//---------------------------------------------------------------------------
bool JNIHelper::ReadFile(const char* fileName, std::vector<uint8_t>* buffer_ref)
{
    if(activity_ == NULL)
    {
...
uint32_t JNIHelper::LoadTexture(const char* file_name)
{
    ...
    mid = env->GetMethodID(
        jni_helper_java_class_, "loadTexture", "(Ljava/lang/String;)Z");
...
int32_t JNIHelper::GetNativeAudioBufferSize()
{
    ...
    mid = env->GetMethodID(
        jni_helper_java_class_, "getNativeAudioBufferSize", "()I");
...
int32_t JNIHelper::GetNativeAudioSampleRate()
{
    ...
    mid = env->GetMethodID(
        jni_helper_java_class_, "getNativeAudioSampleRate", "()I");
    int32_t i = env->CallIntMethod(jni_helper_java_ref_, mid);
    activity_->vm->DetachCurrentThread();
    pthread_mutex_unlock(&mutex_);
