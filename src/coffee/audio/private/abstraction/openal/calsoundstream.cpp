...
CALSoundStream::CALSoundStream(CALSoundDevice& device, const CSoundFormat& fmt)
    : CSoundStream(&device)
    , m_capDev(nullptr)
{
    b_inputStream = false;
...
CALSoundStream::~CALSoundStream()
{
    ...
    source_dequeue_buffers(m_soundSource, m_expended.size(), m_expended.data());
...
void CALSoundStream::feedData(
    c_cptr data, const CSoundFormat& fmt, const szptr& samples)
{
    CALBuffer* buf;
...
{
    i32 procd =
        source_geti(m_soundSource, CSourceProperty::BuffersProcessed) - 1;
    CALBuffer* buffer;
