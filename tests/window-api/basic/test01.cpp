...
GX_SetDispCopyYScale((f32)gamecube_rmode->xfbHeight/(f32)gamecube_rmode->efbHeight);
GX_SetScissor(0, 0, gamecube_rmode->fbWidth, gamecube_rmode->efbHeight);
GX_SetDispCopySrc(0, 0, gamecube_rmode->fbWidth, gamecube_rmode->efbHeight);
GX_SetDispCopyDst(gamecube_rmode->fbWidth, gamecube_rmode->xfbHeight);
GX_SetCopyFilter(gamecube_rmode->aa, gamecube_rmode->sample_pattern, GX_TRUE, gamecube_rmode->vfilter);

auto screenMode = gamecube_rmode;

GX_SetFieldMode(gamecube_rmode->field_rendering, ((screenMode->viHeight==2*screenMode->xfbHeight)?GX_ENABLE:GX_DISABLE));
...
GX_SetViewport(0,0,gamecube_rmode->fbWidth,gamecube_rmode->efbHeight,0,1);
GX_InvVtxCache();
GX_InvalidateTexAll();
Mtx	modelView;
...
guMtxTransApply(modelView, modelView,
                0.0F,	0.0F, -((i % 50) + 25));
...
bool basic_window_test()
{
    ...
    {
                    EventHandlers::ExitOnQuitSignal<R>,
                    nullptr, &eld->r()
                });
    ...
    if(RuntimeQueue::QueuePeriodic(q, Chrono::seconds(2), []() {auto gc_time = time(NULL); printf("DERP DERP: %s\n", ctime(&gc_time)); }) == 0)
