...
// Darkening away from the convergence point, as an ellipse in texture space:
...
void main()
{
    ...
    for(int i = 0; i < TAPS; i++)
    {
        ...
        accum += texture2D(source, uv).rgb * weight;
        total += weight;
    }

    // Distance measured in units of the ellipse's own axes, so 1.0 is its edge
    float radius  = length(offset / falloff_extent);
    float falloff = pow(max(0.0, 1.0 - radius), falloff_power);

    gl_FragColor = vec4(accum / total * exposure * falloff, 1.0);
}
