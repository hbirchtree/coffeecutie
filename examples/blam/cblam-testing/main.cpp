...
//    cstring mapstring = GetArgument(argv,argc,"halomap");
    ...
    cDebug("Opening map: {0}, build {1}",
           C_CAST<cstring>(map), map.map->buildDate);
    ...
    auto scenario = *std::find_if(index_view.begin(),
                                  index_view.end(), pred);

    cDebug("Predicate test: {0} -> {1}",
           scenario->tagId, map.get_name(scenario));
    ...
    auto event = Display::MkEventLoop(
    ...
    event->setup = [&](Display::RendererInterface& r, Empty*)
    {
        ...
        for(auto bitm  : texture_vec)
        {
            ...
            auto head = bitm->to_reflexive<bitm_header_t>()
            ...
            for(auto i : Range<i32>(head->imageCount))
            {
                auto image = head->image_headers()
                        .data(mapfile.data,
                              index_view.tags()->index_magic);

                auto texture = bitm_get_texture(image,
                                                bitmfile.data);
...
//                    && img_h->isize.area() != 0)
//            {
//                auto img_data = r->image_ptr(img_h).data(bitmfile.data, 0);
