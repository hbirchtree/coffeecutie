...
inline std::vector<u8> gzip_decompress(const u8* data, size_t len)
{
    ...
    if(hdr + 8 > len)
        throw std::runtime_error("gzip body too short");
...
inline void underlay_set_hsl(Underlay& u)
{
    ...
    if(mx != mn)
    {
        sat = light < 0.5 ? (mx - mn) / (mx + mn) : (mx - mn) / (2.0 - mx - mn);
        if(mx == r)
            hue = (g - b) / (mx - mn);
        else if(mx == g)
            hue = 2.0 + (b - r) / (mx - mn);
...
inline int noise(int x, int y)
{
    ...
    int n = x + y * 57;
    n     = (n << 13) ^ n;
    return ((n * (n * n * 15731 + 789221) + 1376312589) & 0x7fffffff) >> 19 &
           0xFF;
}
...
// Result is clamped to [0, 255]; average terrain ≈ 35 tile units = 280 game
// units.
inline int ground_height(int wx, int wy)
{
    // scale=4 and scale=2 only — scale=1 is per-tile noise that looks jagged.
    // Amplitudes calibrated for gentle rolling hills (±20 tile units from base
    // 35).
            ...
            for(int y = 0; y < REGION_SIZE; ++y)
            {
                ...
                while(pos < raw.size())
                {
                    ...
                    else if(v == 1)
                    {
                        ...
                        t.height =
                            plane == 0 ? h : planes[plane - 1][x][y].height + h;
    ...
    const std::vector<u8>& raw, int region_x, int region_y)
{
    ...
    while(pos < raw.size())
    {
        ...
        while(pos < raw.size())
        {
            ...
            int local_x =
                (packed >> 6) & 0x3F; // tile x within region (east-west)
            int plane = packed >> 12;
    ...
    const std::vector<u8>& d, size_t pos, size_t end, LocDef& def)
{
    ...
    while(pos < end)
    {
        ...
        case 40: {
            ...
            for(int j = 0; j < n && pos + 4 <= end; ++j, pos += 4)
                def.recolor.emplace_back(
                    read_u16_be(d.data() + pos),
                    read_u16_be(d.data() + pos + 2));
            break;
        }
        case 41:
            pos += size_t(d[pos]) * 4 + 1;
            break; // retexture (later revs)
        case 44:
        case 45:
            pos += 2;
            break;
        ...
        case 64:
            break;
        case 65:
            def.model_size_x = read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 66:
            def.model_size_h = read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 67:
            def.model_size_y = read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 68:
            pos += 2;
        ...
        case 69:
            ...
            break;
        case 70:
            def.offset_x = (int16_t)read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 71:
            def.offset_h = (int16_t)read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 72:
            def.offset_y = (int16_t)read_u16_be(d.data() + pos);
            pos += 2;
            break;
        case 73:
        case 74:
...
inline RsModel decode_model(const ModelSrc& s)
{
    ...
    m.verts.resize(s.nv);
    {
        ...
        for(int i = 0; i < s.nv; ++i)
        {
            ...
            if(flags & 1)
                x += read_smart2(s.vx, px);
            if(flags & 2)
                y += read_smart2(s.vy, py);
            if(flags & 4)
                z += read_smart2(s.vz, pz);
            m.verts[i] = {x, y, z};
        }
    }
    ...
    m.tex_coord.assign(s.nf, -1);
    {
        ...
        for(int i = 0; i < s.nf; ++i)
        {
            u32 col = read_u16_be(s.fcolor + pc);
            pc += 2;
            if(s.texflag)
            {
...
inline RsModel parse_model(const std::vector<u8>& raw)
{
    ...
    size_t off        = 0;
    size_t off_vflags = off;
    off += nv;
    size_t off_ctypes = off;
    off += nf;                         // face compression types
    off += (priority == 255) ? nf : 0; // face priorities (unused here)
    off += has_fskins ? nf : 0;
    size_t off_texflag = off;
    off += uses_tex ? nf : 0;
    off += has_vskins ? nv : 0;
    size_t off_falpha = off;
    off += has_alpha ? nf : 0;
    size_t off_fidx = off;
    off += fi_len;
    size_t off_fcolor = off;
    off += size_t(nf) * 2;
    size_t off_texmap = off;
    off += size_t(ntex) * 6; // texture P/M/N triples
    size_t off_vx = off;
    off += x_len;
    size_t off_vy = off;
    off += y_len;
...
struct ObModelArchive
{
    ...
    bool load(const std::vector<u8>& archive)
    {
        ...
        if(!need("ob_head.dat", head) || !need("ob_point1.dat", point1) ||
           ...
           !need("ob_point4.dat", point4) || !need("ob_vertex1.dat", vertex1) ||
        ...
        for(int n = 0; n < count; ++n)
        {
            ...
            for(int i = 0; i < m.nv; ++i)
            {
                ...
                int flags = point1[p1++];
                if(flags & 1)
                    read_smart2(point2.data(), p2);
                if(flags & 2)
                    read_smart2(point3.data(), p3);
                if(flags & 4)
                    read_smart2(point4.data(), p4);
            }
            ...
            col += size_t(m.nf) * 2;
            tex += m.ntex;
            if(p2 > point2.size() || p3 > point3.size() || p4 > point4.size() ||
               v1 > vertex1.size() || col > face1.size() ||
               tex * 6 > axis.size())
                return false;
...
inline void model_face_uvs(const RsModel& m, size_t fi, float* uu, float* vv)
{
    ...
    double d0x = vx(fc[0]) - vx(p), d0y = vy(fc[0]) - vy(p),
           d0z = vz(fc[0]) - vz(p);
    double d1x = vx(fc[1]) - vx(p), d1y = vy(fc[1]) - vy(p),
           d1z = vz(fc[1]) - vz(p);
    double d2x = vx(fc[2]) - vx(p), d2y = vy(fc[2]) - vy(p),
           d2z = vz(fc[2]) - vz(p);
    ...
    const std::vector<u8>& body, const std::vector<u8>& idx, int id)
{
    ...
    std::vector<std::array<u8, 4>> palette(std::max(pal_size, 1), {0, 0, 0, 0});
    size_t                         p = ioff + 5;
    for(int i = 1; i < pal_size; ++i, p += 3)
    {
        if(p + 3 > idx.size())
            return std::nullopt;
        palette[i] = {
            brighten_channel(idx[p]),
            brighten_channel(idx[p + 1]),
            brighten_channel(idx[p + 2]),
            255};
    }
                ...
                if(ioff + 5 <= idx.size())
                {
                    ...
                    if(pal_size > 0)
                    {
                        ...
                        auto bright = [&](u32 sum) {
                            ...
                            int    v =
                                int(std::pow(std::max(0.0, c), 0.8) * 256.0);
                            return (u8)std::min(255, std::max(0, v));
                        };
...
inline std::vector<int> model_vertex_lights(const RsModel& m)
{
    ...
    for(const auto& fc : m.faces)
    {
        ...
        int len =
            int(std::sqrt(double(nx) * nx + double(ny) * ny + double(nz) * nz));
        if(len <= 0)
            len = 1;
        nx = nx * 256 / len;
        ny = ny * 256 / len;
    ...
    RsModel&      out)
{
    ...
    for(int id : *model_ids)
    {
        ...
        out.verts.insert(
            out.verts.end(), part->verts.begin(), part->verts.end());
