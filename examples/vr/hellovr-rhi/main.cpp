...
void dprintf( const char *fmt, ... )
{
        ...
        va_start( args, fmt );
        out = Coffee::cStringFormat(fmt,args);
        va_end( args );
...
bool CMainApplication::BInit()
{
        ...
        {
                m_pHMD = NULL;
                Coffee::CString out = Coffee::cStringFormat(
                            "Unable to init VR runtime: {0}",
                            vr::VR_GetVRInitErrorAsEnglishDescription( eError ));
                SDL_ShowSimpleMessageBox( SDL_MESSAGEBOX_ERROR, "VR_Init Failed", out.c_str(), NULL );
                return false;
        }
        ...
        if( !m_pRenderModels )
        {
                ...
                Coffee::CString out = Coffee::cStringFormat(
                            "Unable to get render model interface: {0}",
                            vr::VR_GetVRInitErrorAsEnglishDescription( eError ));
                SDL_ShowSimpleMessageBox( SDL_MESSAGEBOX_ERROR, "VR_Init Failed", out.c_str(), NULL );
                return false;
        }
        ...
        Coffee::CGL::CGL43::LoadBinding(nullptr);
