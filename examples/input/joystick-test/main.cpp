...
Sprites::Texture sprite_load(
    Sprites*               instance,
    Sprites::Renderer*     renderer,
    cstring                source,
    Sprites::SpriteSource* size)
{
    Sprites::Texture out;
...
i32 coffee_main(i32, cstring_w*)
{
    ...
    /* Create sprites */
    Sprites::Texture controller_atlas =
        sprite_load(&rend, &inst, "atlas.png", nullptr);
    ...
    auto bind_sprite = [&](Sprites::SpriteSource const& src,
    ...
    auto draw_element = [&](Sprites::Sprite const& unpressed,
        ...
        if(background)
            rend.drawSprite(
                inst,
                {base.x() * size.x(), base.y() * size.y()},
                {size.x(), size.y()},
                *background);
        rend.drawSprite(
            inst,
            {(base.x() + pos.x()) * size.x(), (base.y() + pos.y()) * size.y()},
            {size.x(), size.y()},
            *sp);
    };
    ...
    auto draw_axis = [&](Vecf2 base, i16 x, i16 y, bool btn, f32 scale) {
        draw_element(
            ...
            base,
            {scale},
            {scalar(scalar(x) / Int16_Max * 128.f + 64),
    ...
    while(!winhost.closeFlag())
    {
...

        {
            ...
            draw_element(
                ...
                &trigger_back_sprite,
                false,
                {back_root.x(), back_root.y() + 64},
                {scale_factor},
                {0, scalar(scalar(ctl.axes.e.t_l) / Int16_Max * 220.f)});

            Vecf2 right_anchor = {scalar(win_size.w - 128) + back_root.x(),
                                  back_root.y()};
            draw_element(
                trigger_sprite,
                nullptr,
                &trigger_back_sprite,
                false,
                {right_anchor.x(), right_anchor.y() + 64},
                {scale_factor},
            ...
            draw_element(
                ...
                nullptr,
                ctl.buttons.e.a,
                {button_root.x() + 96, button_root.y() + 196},
                {scale_factor},
                {});
            draw_element(
                button_x_sprite,
                ...
                nullptr,
                ctl.buttons.e.x,
                {button_root.x(), button_root.y() + 96},
                {scale_factor},
                {});
            draw_element(
                button_b_sprite,
                ...
                nullptr,
                ctl.buttons.e.b,
                {button_root.x() + 192, button_root.y() + 96},
                {scale_factor},
                {});
            draw_element(
                button_y_sprite,
                ...
                nullptr,
                ctl.buttons.e.y,
                {button_root.x() + 96, button_root.y()},
                {scale_factor},
                {});
            ...
            draw_element(
                ...
                nullptr,
                ctl.buttons.e.guide,
                {menu_root.x() - 48.f, menu_root.y() - 64.f},
                {scale_factor},
                {});
        }
