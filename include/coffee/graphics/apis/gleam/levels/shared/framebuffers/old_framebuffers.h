...
struct CGL_Old_Framebuffers
{
    ...
    {
        glBlitFramebuffer(
            s.x, s.y, s.w, s.h, d.x, d.y, d.w, d.h, to_enum2(m), to_enum(f));
    }
#endif
    ...
    STATICINLINE void FBReadPixels(
        int32 x, int32 y, uint32 w, uint32 h, PixFmt fmt, BitFormat dt, c_ptr p)
    {
        glReadPixels(
            x, y, w, h, to_enum(GetPixComponent(fmt), fmt), to_enum(dt), p);
    }
    ...
    {
        CGenum fmt = GL_NONE;
        if(ifmt == PixelFormat::Depth || ifmt == PixelFormat::DepthStencil)
            fmt = to_enum(ifmt);
        else
            fmt = GL_RGBA;
        ...
        PixelFormat ifmt, uint32 samples, uint32 w, uint32 h)
    {
        glRenderbufferStorageMultisample(
            GL_RENDERBUFFER, samples, to_enum(ifmt), w, h);
    }
#endif
