...
{
    /* Textures */
    STATICINLINE void TexBind(Texture t, CGhnd h)
    {
        glBindTexture(to_enum(t), h);
    }
    STATICINLINE void TexActive(uint32 i)
    {
    ...
    /* TexGet */
    STATICINLINE void TexGetImageSize(
        Texture t,
        PixCmp  c,
        uint32& w,
        uint32& h,
        ...
        uint32  mip = 0)
    {
        ...
        case PixCmp::RGBA:
            cmp_size +=
                TexGetLevelParameteri(t, mip, GL_TEXTURE_ALPHA_SIZE) / 8;
        case PixCmp::RGB:
            cmp_size += TexGetLevelParameteri(t, mip, GL_TEXTURE_BLUE_SIZE) / 8;
        case PixCmp::RG:
            cmp_size +=
                TexGetLevelParameteri(t, mip, GL_TEXTURE_GREEN_SIZE) / 8;
        case PixCmp::R:
        case PixCmp::G:
        case PixCmp::B:
        ...
        case PixCmp::DepthStencil:
            cmp_size +=
                TexGetLevelParameteri(t, mip, GL_TEXTURE_STENCIL_SIZE) / 8;
        case PixCmp::Depth:
            cmp_size +=
                TexGetLevelParameteri(t, mip, GL_TEXTURE_DEPTH_SIZE) / 8;
            break;
        case PixCmp::Stencil:
            cmp_size +=
                TexGetLevelParameteri(t, mip, GL_TEXTURE_STENCIL_SIZE) / 8;
    ...
    STATICINLINE void TexCopyImage2D(
        Texture     t,
        uint32      level,
        PixelFormat fmt,
        int32       x,
    ...
    STATICINLINE void TexGetLevelParameteriv(
        Texture t, uint32 l, CGenum f, int32* v)
    {
    ...
    STATICINLINE void TexGetLevelParameterfv(
        Texture t, uint32 l, CGenum f, scalar* v)
    {
        glGetTexLevelParameteriv(to_enum(t), l, f, v);
    }
    ...
    STATICINLINE void SamplerParameteri(
        CGhnd h, CGenum f, Filtering d1, Filtering d2 = Filtering::None)
    {
